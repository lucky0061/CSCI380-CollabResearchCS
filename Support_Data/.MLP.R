#MLP,hidden=c(25,15,10,5),features=44
TradeSignal=function(sec_names){
  ...
  Signal=paste(idNumber,"_TradeSignal_MLP",".csv",sep = "")
  N=length(Datafiles)
  for(j in 1:N){
    ...
    for(i in 1:350){
      ...
      model=deepnet::nn.train(as.matrix(train_set[,-1]),train_set$Label,hidden = c(25,15,10,5))
