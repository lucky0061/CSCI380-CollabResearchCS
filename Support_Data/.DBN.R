#Deep Belief Network,hidden=c(25,15,10,5),features=44
TradeSignal=function(sec_names){
  ...
  Signal=paste(idNumber,"_TradeSignal_DBN",".csv",sep = "")
  N=length(Datafiles)
  for(j in 1:N){
    ...
    for(i in 1:350){
      ...
      model=deepnet::dbn.dnn.train(as.matrix(train_set[,-1]),train_set$Label,hidden = c(25,15,10,5))
